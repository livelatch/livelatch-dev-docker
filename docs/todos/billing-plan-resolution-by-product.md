...
ask_after: 2026-06-23T12:00:00+08:00
status: open
-->

Today `StripePlanResolver::planForSubscription()` decides "is this Pro?" by
matching a single price ID (`config('billing.pro_price_id')` ← env
...
price would otherwise mean adding/maintaining another env var per price — the
wrong pattern.
...
- **Preferred — product-based:** map one Stripe **Product** ("Livelatch Pro") →
...
- **Alternative — metadata convention:** set `metadata.plan_key = "pro"` on the
...
- [ ] Update `app/Services/StripePlanResolver.php` to match on
      `price->product` instead of `price->id`. (Single change point — the webhook
