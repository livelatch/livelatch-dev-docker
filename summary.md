...
- Validation: `php -l` clean on all changed PHP. Migrations not runnable from host (no `pdo_mysql`/docker) — run `php artisan migrate` on Railway/in-container; the backfill migration seeds + populates everything in one step.
