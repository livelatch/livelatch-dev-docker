...
- Follow-ups (same day): (1) dropped xterm.js — its FitAddon mis-sized inside the HTMX studio layout and blew out the page; replaced with a plain width-constrained `<pre>` console (colored spans for command/error/meta, no CDN). (2) Fixed a CSS grid blowout pushing content left under the sidebar — `.ll-shell` grid children needed `min-width: 0` (grid items default to `min-width: auto`, letting the `<pre>` force a track wider than the viewport). (3) Added a **Favourites** bar: save commands as removable chips (localStorage `ll_shell_favourites`, per-browser); click a chip to run it immediately, `×` to remove.
