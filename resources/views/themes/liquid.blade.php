@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#ff5e8a';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#7a5bff';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#07030f';
    ...
    $themeJs    = is_file(public_path('assets/themes/liquid/liquid.js'))    ? file_get_contents(public_path('assets/themes/liquid/liquid.js'))    : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'point to push the liquid'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/liquid/liquid.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.LiquidBG) LiquidBG.init(window.__llx); });</script>
