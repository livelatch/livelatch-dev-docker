@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#ffe39a';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#9be86a';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#06100a';
    $textColor= preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['textColor'] ?? '') ? $s['textColor'] : '#eafff0';
    $intensity = max(0, min(100, (int) ($s['intensity'] ?? 50)));
    $speed     = max(0, min(100, (int) ($s['speed'] ?? 50)));
    $headingFont = preg_match('/^[A-Za-z0-9 ]{2,40}$/', $s['headingFont'] ?? '') ? $s['headingFont'] : 'Sora';
    ...
    $themeJs    = is_file(public_path('assets/themes/fireflies/fireflies.js')) ? file_get_contents(public_path('assets/themes/fireflies/fireflies.js')) : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'they follow your pointer'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/fireflies/fireflies.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.FirefliesBG) FirefliesBG.init(window.__llx); });</script>
