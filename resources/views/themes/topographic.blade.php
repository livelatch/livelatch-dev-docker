@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#6fe3ff';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#2a6bff';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#04101a';
    ...
    $themeJs    = is_file(public_path('assets/themes/topographic/topographic.js')) ? file_get_contents(public_path('assets/themes/topographic/topographic.js')) : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'point to send a ripple'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/topographic/topographic.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.TopographicBG) TopographicBG.init(window.__llx); });</script>
