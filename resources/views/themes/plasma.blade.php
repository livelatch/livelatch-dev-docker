@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#ff4ad4';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#2de2e6';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#0a0220';
    ...
    $themeJs    = is_file(public_path('assets/themes/plasma/plasma.js'))    ? file_get_contents(public_path('assets/themes/plasma/plasma.js'))    : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'point to ripple the field'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/plasma/plasma.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.PlasmaBG) PlasmaBG.init(window.__llx); });</script>
