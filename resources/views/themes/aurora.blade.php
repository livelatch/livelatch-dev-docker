@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#5fe6c0';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#6a8bff';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#03060f';
    $textColor= preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['textColor'] ?? '') ? $s['textColor'] : '#eaf6ff';
    ...
    $themeJs    = is_file(public_path('assets/themes/aurora/aurora.js'))    ? file_get_contents(public_path('assets/themes/aurora/aurora.js'))    : null;
...
<head>
  ...
  <style>
    :root {
      --llx-a: {{ $aColor }}; --llx-b: {{ $bColor }}; --llx-bg: {{ $bgColor }}; --llx-text: {{ $textColor }};
      --llx-heading-font: "{{ $headingFont }}", system-ui, sans-serif; --llx-body-font: "{{ $bodyFont }}", system-ui, sans-serif;
    }
  </style>
...
<body>
  <canvas id="llx-canvas" aria-hidden="true"></canvas>
  <div class="llx-vignette" aria-hidden="true"></div>
  @include('themes.partials.glasscard', ['hint' => 'move to stir the lights'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/aurora/aurora.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.AuroraBG) AuroraBG.init(window.__llx); });</script>
