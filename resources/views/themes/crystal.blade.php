@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#8fd0ff';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#c89bff';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#060814';
    $textColor= preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['textColor'] ?? '') ? $s['textColor'] : '#eef3ff';
    ...
    $themeJs    = is_file(public_path('assets/themes/crystal/crystal.js'))   ? file_get_contents(public_path('assets/themes/crystal/crystal.js'))   : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'move to turn the gem'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/crystal/crystal.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.CrystalBG) CrystalBG.init(window.__llx); });</script>
