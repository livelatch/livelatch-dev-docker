@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#ffd9a0';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#ff5e2e';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#02030a';
    $textColor= preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['textColor'] ?? '') ? $s['textColor'] : '#ffe9dc';
    ...
    $themeJs    = is_file(public_path('assets/themes/blackhole/blackhole.js')) ? file_get_contents(public_path('assets/themes/blackhole/blackhole.js')) : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'move to tilt the disk'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/blackhole/blackhole.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.BlackholeBG) BlackholeBG.init(window.__llx); });</script>
