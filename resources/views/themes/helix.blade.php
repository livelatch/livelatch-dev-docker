@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#36e0ff';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#ff5ea8';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#04060f';
    ...
    $themeJs    = is_file(public_path('assets/themes/helix/helix.js'))     ? file_get_contents(public_path('assets/themes/helix/helix.js'))     : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'move to spin the helix'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/helix/helix.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.HelixBG) HelixBG.init(window.__llx); });</script>
