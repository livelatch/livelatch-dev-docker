@php
    $s = $settings ?? [];
    $aColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['aColor']    ?? '') ? $s['aColor']    : '#ff2e8b';
    $bColor   = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bColor']    ?? '') ? $s['bColor']    : '#ffcf3c';
    $bgColor  = preg_match('/^#[0-9a-fA-F]{3,8}$/', $s['bgColor']   ?? '') ? $s['bgColor']   : '#170433';
    ...
    $themeJs    = is_file(public_path('assets/themes/outrun/outrun.js'))   ? file_get_contents(public_path('assets/themes/outrun/outrun.js'))   : null;
...
<body>
  ...
  @include('themes.partials.glasscard', ['hint' => 'steer with your pointer'])
  ...
  @if($themeJs !== null)<script>{!! $themeJs !!}</script>@else<script src="{{ asset('assets/themes/outrun/outrun.js') }}"></script>@endif
  <script>document.addEventListener('DOMContentLoaded', function () { if (window.LLXInteract) LLXInteract.init(); if (window.OutrunBG) OutrunBG.init(window.__llx); });</script>
